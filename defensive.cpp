...
// --- PATHFINDING ---
struct Paths { std::vector<std::vector<int>> dist; std::vector<std::vector<int>> nxt; };
static int euclid_ceil(const GameMap &M, int u, int v) {
  double dx = (double)(M.x[u] - M.x[v]), dy = (double)(M.y[u] - M.y[v]); return (int)std::ceil(std::sqrt(dx * dx + dy * dy));
}
static Paths calculate_paths(const GameMap &M) {
  const int INF = 1e9; Paths P;
  P.dist.assign(M.N, std::vector<int>(M.N, INF)); P.nxt.assign(M.N, std::vector<int>(M.N, -1));
  for (int i = 0; i < M.N; ++i) { P.dist[i][i] = 0; P.nxt[i][i] = i; }
  for (int u = 0; u < M.N; ++u) { 
    for (int v : M.adj[u]) { 
      int w = euclid_ceil(M, u, v); 
  ...
  for (int k = 0; k < M.N; ++k) {
    for (int u = 0; u < M.N; ++u) {
      ...
      for (int v = 0; v < M.N; ++v) { 
        if (P.dist[k][v] == INF) continue;
        int cand = P.dist[u][k] + P.dist[k][v]; 
  ...
  for (int u = 0; u < M.N; ++u) {
    for (int v = 0; v < M.N; ++v) {
      ...
      for (int nb : M.adj[u]) {
        ...
        int score = euclid_ceil(M, u, nb) + P.dist[nb][v];
        if (score < best_score || (score == best_score && best_nxt != -1 && nb < best_nxt)) { 
            best_score = score; 
            best_nxt = nb; 
        }
      }
...
public:
    ...
    void update_enemy_attack_groups(const GameState &S, const GameMap &M, const Paths &P) {
        ...
        for (const auto& [path, ids] : moves) {
            bool is_heading_to_enemy_labor = false;
            
            if (P.nxt[path.first][M.opp_hq] == path.second) {
                is_heading_to_enemy_labor = true;
            } else {
                for (const auto& b : S.buildings) {
                    if (b.side != M.my_side && b.type == BType::BASE) {
                        if (P.nxt[path.first][b.region] == path.second) {
                            is_heading_to_enemy_labor = true;
                            break;
                        }
                    }
            ...
            if (ids.size() >= 2) {
                is_threat = true; 
            } else if (ids.size() == 1 && !is_heading_to_enemy_labor) {
                is_threat = true; 
            }
