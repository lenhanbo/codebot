...
// Variant: five_warrior_hq_rush
// Plan:
//   - Start with 3 warriors at HQ.
//   - TRAIN 1 per turn until we have at least 5 warriors.
//   - Once we have 5, send every stationary warrior directly to enemy HQ.
//   - No expansion, no worker preservation, no defense. This is a pure rush bot.

static int count_my_warriors(const GameState &S, const GameMap &M) {
  int cnt = 0;
  for (const auto &w : S.warriors)
    if (w.id.side == M.my_side)
      ++cnt;
...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  int my_count = count_my_warriors(S, M);

  // HQ level 1 can train only 1 warrior per turn. Starting gold is enough
  // to train from 3 -> 5 before rushing.
  if (!launched && my_count < 5 && S.gold >= TRAIN_COST) {
    a.train_n = 1;
    return a;
  }

  // Once we have 5 warriors, launch the all-in HQ rush.
  if (!launched && my_count >= 5) {
    for (const auto &w : S.warriors) {
  ...
  // After launch, any newly stationary own warrior that is not at enemy HQ
  // keeps moving toward enemy HQ. Usually there are no new warriors, but this
  // makes the bot robust if a move command was missed or interrupted.
  for (const auto &w : S.warriors) {
    if (w.id.side != M.my_side) continue;
    if (w.state != WState::STATIONARY) continue;
    if (w.region == M.opp_hq) continue;
